...
fn partner(programs: &mut [char], program_1: char, program_2: char) {
    let position_1 = programs
        .iter()
        .position(|program| *program == program_1)
        .unwrap();
    let position_2 = programs
        .iter()
        .position(|program| *program == program_2)
        .unwrap();
    programs[position_1] = program_2;
    programs[position_2] = program_1;
}
...
fn main() {
    ...
    for i in 0..1_000_000_000 {
        for step in content.trim().split(',') {
            match &step[0..1] {
                ...
                "x" => {
                    let positions: Vec<usize> = step[1..]
                        .split('/')
    ...
    println!(
        "After 1 iteration: {}",
        programs_combinations[1].iter().collect::<String>()
    );
    println!(
